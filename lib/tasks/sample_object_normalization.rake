...
namespace :tufts_data do
  ...
  desc 'migrate sample records'
  task :migrate_sample_records, [:arg1] => :environment do |t, args|
    if args[:arg1].nil?
      puts "YOU MUST SPECIFY FULL PATH TO FILE, ABORTING!"
      puts "Usage: rake tufts_data:migrate_sample_records['/home/hydradm/path_to/list_of_pids.txt']"
      next
    end
